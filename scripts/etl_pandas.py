...
class ETL_Final(ETL_Pandas):
    ...
    def _extract(self) -> dict:
        ...
        df_tipo_cambio = pd.DataFrame(
            data_tipo_cambio, columns=["date", "tipo_cambio_bna_vendedor"]
        )
        ...
        dataframes = {"tipo_de_cambio": df_tipo_cambio, "ipc": df_ipc}
    ...
    def _check_tipo_cambio(self, df_tipo_cambio: pd.DataFrame) -> int:
        ...
        for i in df_filtered.index:
            if i == 0:
                ...
                variacion = round(((df_filtered['tipo_cambio_bna_vendedor'][i] / 
                                    df_filtered['tipo_cambio_bna_vendedor'][i - 1]) - 1), 3)
    ...
    def _df_a_mensual(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        dataframe_mensual["date"] = [
            dt.datetime.strptime(fecha, "%Y-%m-%d") for fecha in dataframe["date"]
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "dolar_variation_limit",
        type=float,
        help="Dolar Variation limit to be considered important",
    )
